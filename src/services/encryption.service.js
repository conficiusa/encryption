...
const {
	...
	connectSecureClient,
	coll,
} = require("../utils/encryptionClient");
const { formSchema } = require("../utils/schema");
const { NotFoundError } = require("../utils/errors");
const { ObjectId } = require("mongodb");
const { encryptionFields } = require("../utils/encryptedFields");
	...
	async createEncryption(data) {
		...
		if (processedReport.clinicalInvestigations?.lab_tests?.length) {
			processedReport.clinicalInvestigations.lab_tests =
				processedReport.clinicalInvestigations.lab_tests.map(
					(item) => item.value
				);
		}
		...
		if (processedReport.clinicalInvestigations?.imaging?.length) {
			processedReport.clinicalInvestigations.imaging =
				processedReport.clinicalInvestigations.imaging.map(
					(item) => item.value
				);
		}
		...
		const writeResult = await coll.insertOne(encryptedRecord);
		return writeResult;
	}
